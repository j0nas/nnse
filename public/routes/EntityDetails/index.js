...
export default class EntityDetails extends React.Component {
    ...
    render() {
        ...
        const editBtn = <button className="btn btn-primary" onClick={() => this.setState({editing: true})}>Endre</button>;
        const cancelEditBtn = <button className="btn btn-danger" onClick={() => this.setState({editing: false})}>Avbryt</button>;
        const deleteButton = <button className="btn btn-danger" onClick={() => this.deleteEntity()}>Slett</button>;
